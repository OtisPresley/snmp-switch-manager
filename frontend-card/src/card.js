...
// Optional panel visibility
  ...
  set hass(hass) {
    ...
    // If a port dialog is open, keep its elements in sync with live state
    if (this._modalEl && this._modalEntityId) {
      ...
      // Keep PoE status label and priority select in sync
      const baseObj = (this._modalEntityId || "").split(".")[1] || "";
      const poeEntityId = `switch.${baseObj}_poe`;
      const poePriorityEntityId = `select.${baseObj}_poe_priority`;

      const poeSt = hass?.states?.[poeEntityId];
      const poeLbl = this._modalEl.querySelector('.poe-status-label');
      if (poeLbl && poeSt) {
        const poeIsOn = (poeSt.state || "").toLowerCase() === "on";
        poeLbl.textContent = poeIsOn ? "PoE: Auto/On" : "PoE: Disabled/Off";
        const poeToggleBtn = this._modalEl.querySelector('[data-poe-toggle]');
        if (poeToggleBtn) {
          poeToggleBtn.textContent = poeIsOn ? "Turn off" : "Turn on";
        }
      }

      const poePriSel = this._modalEl.querySelector('.poe-priority-select');
      const poePriSt = hass?.states?.[poePriorityEntityId];
      if (poePriSel && poePriSt && !poePriSel.matches(':focus')) {
        poePriSel.value = poePriSt.state || "Low";
      }
    }
  ...
  _getDiagnosticsEntityIds() {
    ...
    const diagPrefix = prefix.replace(/_[a-z0-9]+\d+(_\d+)?$/i, "");

    // Built-in diagnostic keys mapped to entity suffixes
    const builtin = ["hostname", "manufacturer", "model", "firmware_revision", "system_contact", "system_location", "uptime"];
    ...
    const hasCustomOrder = (Array.isArray(this._config?.diagnostics_order) && this._config.diagnostics_order.length) ||
                            (Array.isArray(this._config?.diagnostics) && this._config.diagnostics.length);

    const order = rawOrder;

    // Try to find consolidated device info sensor
    let devInfoEid = `sensor.${diagPrefix}_device_info`;
    if (!H[devInfoEid]) {
      devInfoEid = `sensor.${diagPrefix}_device_information`;
    }
    if (!H[devInfoEid]) {
      devInfoEid = `sensor.${prefix}_device_info`;
    }
    if (!H[devInfoEid]) {
      devInfoEid = `sensor.${prefix}_device_information`;
    }
    if (!H[devInfoEid]) {
      devInfoEid = Object.keys(H).find(k => 
        k.startsWith("sensor.") && 
        (k.includes(diagPrefix) || k.includes(prefix)) && 
        (k.endsWith("_device_info") || k.endsWith("_device_information"))
      ) || null;
    }
    ...
    for (const raw of order) {
      ...
      if (enabledMap[mapped] === false) continue;

      // If we have the consolidated device info sensor, resolve to its attributes
      if (devInfoEid && H[devInfoEid]) {
        const attrMap = {
          hostname: "Hostname",
        ...
        const attr = attrMap[mapped];
        if (attr) {
          // Verify attribute exists case-insensitively
          const attrs = H[devInfoEid].attributes || {};
          const actualKey = Object.keys(attrs).find(k => k.toLowerCase() === attr.toLowerCase());
          if (actualKey) {
            out.push(`${devInfoEid}#${actualKey}`);
            continue;
          }
        }
      }

      // Fallback: look for individual sensors (v0.5.x and earlier)
      const suffixes = {
        hostname: ["_hostname"],
        manufacturer: ["_manufacturer"],
        model: ["_model"],
        firmware_revision: ["_firmware_revision", "_firmware", "_firmware_version"],
        system_contact: ["_system_contact", "_contact"],
        system_location: ["_system_location", "_location"],
        uptime: ["_uptime"]
      };

      const targets = suffixes[mapped] || ["_" + mapped];
      let resolved = false;
      for (const suffix of targets) {
        const eid = `sensor.${diagPrefix}${suffix}`;
        if (H[eid]) {
          out.push(eid);
          resolved = true;
          break;
        }
        const eid2 = `sensor.${prefix}${suffix}`;
        if (H[eid2]) {
          out.push(eid2);
          resolved = true;
          break;
      ...
      if (!resolved) {
        // Try searching H for any key ending in the suffix
        for (const suffix of targets) {
          const eid = Object.keys(H).find(k => k.startsWith("sensor.") && (k.includes(diagPrefix) || k.includes(prefix)) && k.endsWith(suffix));
          if (eid) {
            out.push(eid);
            break;
          }
  ...
  _stripDiagPrefix(name) {
    ...
    const diagPrefix = p.replace(/_[a-z0-9]+\d+(_\d+)?$/i, "");
    const candidates = [];
    ...
    if (diagPrefix && diagPrefix !== p) {
      candidates.push(diagPrefix);
      candidates.push(diagPrefix.replace(/_/g, "-"));
      candidates.push(diagPrefix.replace(/_/g, " "));
      candidates.push(diagPrefix.replace(/_/g, "."));
      candidates.push(diagPrefix.replace(/-/g, "_"));
      candidates.push(diagPrefix.replace(/-/g, "."));
      candidates.push(diagPrefix.replace(/-/g, " "));
    }
  ...
  _autoDefaultDiagKeys(prefix, H) {
    ...
    const tryKeys = (suffix) => {
      const e1 = `sensor.${diagPrefix}${suffix}`;
      if (H[e1]) return e1;
      const e2 = `sensor.${prefix}${suffix}`;
      if (H[e2]) return e2;
      return null;
    };

    const tryAttrs = (baseSuffix, attrName) => {
      const e1 = `sensor.${diagPrefix}${baseSuffix}`;
      const st1 = H[e1];
      if (st1 && st1.attributes?.[attrName] != null) return `${e1}#${attrName}`;
      
      const e2 = `sensor.${prefix}${baseSuffix}`;
      const st2 = H[e2];
      if (st2 && st2.attributes?.[attrName] != null) return `${e2}#${attrName}`;
    ...
    // Environment: System Temperature
    const tempEid = tryKeys("_system_temperature") || tryAttrs("_environment", "System Temperature (°C)") || tryAttrs("_environment", "system_temperature");
    if (tempEid) out.push(tempEid);

    // Environment: System Temperature Status
    const tempStatusEid = tryKeys("_system_temperature_status") || tryAttrs("_environment", "System Temperature Status") || tryAttrs("_environment", "system_temperature_status");
    if (tempStatusEid) out.push(tempStatusEid);

    // PoE: Power Used (W)
    const poeUsedEid = tryKeys("_poe_power_used") || tryAttrs("_power_over_ethernet", "PoE Power Used (W)") || tryAttrs("_power_over_ethernet", "poe_power_used");
    if (poeUsedEid) out.push(poeUsedEid);

    // PoE: Power Available (W)
    const poeAvailEid = tryKeys("_poe_power_available") || tryAttrs("_power_over_ethernet", "PoE Power Available (W)") || tryAttrs("_power_over_ethernet", "poe_power_available");
    if (poeAvailEid) out.push(poeAvailEid);
  ...
  async _discoverEntities() {
    ...
    if (!explicit && this._config.device) {
      const pref = String(this._config.device);
      const diagPrefix = pref.replace(/_[a-z0-9]+\d+(_\d+)?$/i, "");
      const basePref = `switch.${diagPrefix}_`;
      const keyCount = Object.keys(H).length;
      if (this._cachedDevicePrefix !== pref || this._cachedStatesKeyCount !== keyCount) {
        const ids = [];
        for (const id in H) {
          if (id.startsWith(basePref)) ids.push(id);
        }
    ...
    entries = entries || (explicit
      ...
      : Object.entries(H).filter(([id, st]) => {
          ...
          if (this._config.device) {
            ...
            const diagPrefix = pref.replace(/_[a-z0-9]+\d+(_\d+)?$/i, "");
            const basePref = `switch.${diagPrefix}_`;
            if (!id.startsWith(basePref)) return false;
            return true;
          }
    ...
    for (const [id, st] of entries) {
      ...
      const hasPhysType = portType === "physical" || portType === "phys" || portType.includes("phys");
      const hasVirtType = portType === "virtual" || portType === "virt" || portType.includes("virtual");

      if (!hasPhysType && !hasVirtType) {
        // Not a valid SNMP Switch Manager port interface. Skip it!
        continue;
      }

      // If physical, it must match the slot/interface prefix (if device is configured)
      if (hasPhysType && this._config.device) {
        const pref = String(this._config.device);
        const diagPrefix = pref.replace(/_[a-z0-9]+\d+(_\d+)?$/i, "");
        
        // Extract unit number from device configuration (e.g. "gi1_0" -> unit "1")
        const unitMatch = pref.match(/_([a-z0-9]+?)(\d+)(?:_\d+)?$/i);
        if (unitMatch) {
          const unit = unitMatch[2];
          // Match any interface type followed by the same unit number
          const portRegex = new RegExp(`^switch\\.${diagPrefix}_[^_]+${unit}_`, 'i');
          if (!portRegex.test(id)) {
            continue;
          }
        } else {
          // Fallback: match any port on the same device prefix
          const basePref = `switch.${diagPrefix}_`;
          if (!id.startsWith(basePref)) {
            continue;
          }
        }
      }
      ...
      if (_virtSet && _virtSet.size) {
        ...
        isPhysical = false; // safest default
      }
...
_parseSpeedMbps(attrs) {
  ...
  async _openBandwidthGraphDialog(title, rxEntityId, txEntityId, force = false) {
    ...
    root.innerHTML = `
      ...
      <div class="ssm-modal" role="dialog" aria-modal="true" style="position: relative;">
        <div class="ssm-modal-title">${title} – Bandwidth</div>
        <div class="ssm-modal-body" style="position: relative; min-height: 260px;">
          <div class="ssm-graph-host">Loading…</div>
          <div class="ssm-graph-tooltip" style="position: absolute; display: none; background: color-mix(in srgb, var(--ha-card-background, var(--card-background-color, #1e1e24)) 95%, transparent); border: 1px solid color-mix(in srgb, var(--divider-color, rgba(255, 255, 255, 0.15)) 40%, transparent); border-radius: 8px; padding: 8px 12px; pointer-events: none; font-size: 11px; z-index: 12005; box-shadow: 0 8px 16px rgba(0, 0, 0, 0.4); font-family: var(--primary-font-family, sans-serif); color: var(--primary-text-color); white-space: nowrap; line-height: 1.4;"></div>
        </div>
        <div class="ssm-modal-actions">
          ...
          <div class="ssm-graph-scale-wrap" style="display:flex;align-items:center;gap:6px;margin-right:auto;margin-left:12px;">
            <span class="ssm-graph-scale-label" style="font-size:12px;opacity:.85;">Scale</span>
            <select class="ssm-graph-scale" data-graph-scale="1">
              <option value="auto">Auto</option>
              <option value="bit/s">bps</option>
              <option value="kbit/s">Kbps</option>
              <option value="Mbit/s">Mbps</option>
              <option value="Gbit/s">Gbps</option>
            </select>
          </div>
    ...
    style.textContent = `
      ...
      .ssm-graph-modal-root .ssm-backdrop{position:fixed;inset:0;background:rgba(0, 0, 0, 0.4);backdrop-filter: blur(12px);-webkit-backdrop-filter: blur(12px);z-index:12000;animation: fade-in 0.2s ease-out;}
      .ssm-graph-modal-root .ssm-modal{position:fixed;z-index:12001;left:50%;top:50%;transform:translate(-50%,-50%);
        min-width:320px;width:90vw;max-width:900px;
        background: color-mix(in srgb, var(--ha-card-background, var(--card-background-color, #1e1e24)) 80%, transparent);
        border: 1px solid color-mix(in srgb, var(--divider-color, rgba(255, 255, 255, 0.08)) 30%, transparent);
        color:var(--primary-text-color);border-radius:20px;box-shadow:0 24px 64px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255, 255, 255, 0.05);padding:24px;
        backdrop-filter: blur(24px);-webkit-backdrop-filter: blur(24px);animation: scale-in 0.2s cubic-bezier(0.34, 1.56, 0.64, 1);}
      .ssm-graph-modal-root .ssm-modal-title{font-weight:700;font-size:18px;margin-bottom:8px}
      .ssm-graph-modal-root .ssm-modal-actions{display:flex;gap:8px;justify-content:flex-end;margin-top:12px;align-items:center;}
      .ssm-graph-modal-root .btn{font:inherit;padding:8px 12px;border-radius:10px;
        border:1px solid color-mix(in srgb, var(--primary-text-color) 12%, transparent);
        background:color-mix(in srgb, var(--primary-text-color) 8%, transparent);
        color:var(--primary-text-color);cursor:pointer;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);transition:all 0.2s ease;}
      .ssm-graph-modal-root .btn:hover {
        background:color-mix(in srgb, var(--primary-text-color) 15%, transparent);
        border-color:color-mix(in srgb, var(--primary-text-color) 25%, transparent);
      }
      .ssm-graph-modal-root .btn.subtle{background:transparent;border-color:transparent;}
      .ssm-graph-modal-root .btn.subtle:hover{background:color-mix(in srgb, var(--primary-text-color) 6%, transparent);}
      .ssm-graph-host{min-height:260px;width:100%;}
    
      .ssm-graph-refresh-wrap{display:flex;align-items:center;gap:6px;}
      .ssm-graph-refresh-label{font-size:12px;opacity:.85;}
      .ssm-graph-refresh, .ssm-graph-scale {
        background: color-mix(in srgb, var(--primary-text-color) 8%, transparent);
      ...
      .ssm-graph-refresh:hover, .ssm-graph-scale:hover {
        background: color-mix(in srgb, var(--primary-text-color) 15%, transparent);
        border-color: color-mix(in srgb, var(--primary-text-color) 25%, transparent);
      }
      .ssm-graph-refresh option, .ssm-graph-scale option {
        background: var(--card-background-color, var(--ha-card-background, #1e1e24));
        color: var(--primary-text-color);
      }
    `;
    ...
    const renderGraph = async (forceRebuild = false) => {
      host.innerHTML = `<div style="display:flex; justify-content:center; align-items:center; height:260px; color:var(--secondary-text-color); font-family:var(--primary-font-family, sans-serif);">Loading history data…</div>`;
      try {
        const now = Date.now();
        let history = this._cachedGraphHistory;
        if (forceRebuild || !history || !this._cachedGraphHistoryTime || (now - this._cachedGraphHistoryTime > 15000)) {
          ...
          history = await this._hass.callWS({
            ...
            entity_ids: [rxEntityId, txEntityId],
            include_start_time_state: true,
            significant_changes_only: false,
            no_attributes: true
          });
          this._cachedGraphHistory = history;
          this._cachedGraphHistoryTime = now;
        }

        const rxStates = history[rxEntityId] || [];
        const txStates = history[txEntityId] || [];

        const rxPoints = rxStates.map(pt => ({
          val: parseFloat(pt.s),
          time: (pt.lu ?? pt.t) * 1000
        })).filter(pt => Number.isFinite(pt.val) && Number.isFinite(pt.time));

        const txPoints = txStates.map(pt => ({
          val: parseFloat(pt.s),
          time: (pt.lu ?? pt.t) * 1000
        })).filter(pt => Number.isFinite(pt.val) && Number.isFinite(pt.time));

        if (rxPoints.length === 0 && txPoints.length === 0) {
          host.innerHTML = `<div style="display:flex; justify-content:center; align-items:center; height:260px; color:var(--secondary-text-color); font-family:var(--primary-font-family, sans-serif);">No history data available for the last 24 hours.</div>`;
          return;
        }

        const rxUnit = this._hass?.states?.[rxEntityId]?.attributes?.unit_of_measurement || "bit/s";
        const txUnit = this._hass?.states?.[txEntityId]?.attributes?.unit_of_measurement || "bit/s";

        const convertToBits = (val, unit) => {
          const isBytes = /B|byte|octet/i.test(unit);
          return isBytes ? val * 8 : val;
        };

        // Determine max bits value to select Auto scale
        let maxBitsVal = 0;
        for (const p of rxPoints) {
          maxBitsVal = Math.max(maxBitsVal, convertToBits(p.val, rxUnit));
        }
        for (const p of txPoints) {
          maxBitsVal = Math.max(maxBitsVal, convertToBits(p.val, txUnit));
        }

        let selectedScale = this._selectedGraphScale || "auto";
        let activeScale = selectedScale;
        if (selectedScale === "auto") {
          if (maxBitsVal >= 1000000000) activeScale = "Gbit/s";
          else if (maxBitsVal >= 1000000) activeScale = "Mbit/s";
          else if (maxBitsVal >= 1000) activeScale = "kbit/s";
          else activeScale = "bit/s";
        }

        const convertValue = (val, fromUnit, targetScale) => {
          let valNum = parseFloat(val);
          if (!Number.isFinite(valNum)) return 0;
          const bits = convertToBits(valNum, fromUnit);
          switch (targetScale) {
            case "kbit/s": return bits / 1000;
            case "Mbit/s": return bits / 1000000;
            case "Gbit/s": return bits / 1000000000;
            case "bit/s":
            default: return bits;
          }
        };

        const rxScaled = rxPoints.map(p => ({
          val: convertValue(p.val, rxUnit, activeScale),
          time: p.time
        }));
        const txScaled = txPoints.map(p => ({
          val: convertValue(p.val, txUnit, activeScale),
          time: p.time
        }));

        const scaleDisplayLabels = {
          "bit/s": "bps",
          "kbit/s": "Kbps",
          "Mbit/s": "Mbps",
          "Gbit/s": "Gbps"
        };
        const scaleLabel = scaleDisplayLabels[activeScale] || activeScale;

        // Sort data points
        const rxSorted = rxScaled.sort((a, b) => a.time - b.time);
        const txSorted = txScaled.sort((a, b) => a.time - b.time);
        ...
        let maxYVal = 0;
        for (const p of rxScaled) maxYVal = Math.max(maxYVal, p.val);
        for (const p of txScaled) maxYVal = Math.max(maxYVal, p.val);
        maxYVal = maxYVal > 0 ? maxYVal * 1.15 : 10;

        // Grid lines
        const yGridLines = [];
        for (let i = 0; i <= 4; i++) {
          const frac = i / 4;
          const val = maxYVal * frac;
        ...
        for (let i = 0; i <= 4; i++) {
          const frac = i / 4;
          const time = minTime + timeRange * frac;
          const x = padL + chartW * frac;
          const d = new Date(time);
          const timeStr = d.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false });
          xTicks.push({ x, timeStr });
        }

        const getX = (t) => padL + chartW * Math.max(0, Math.min(1, (t - minTime) / timeRange));
        const getY = (v) => padT + chartH * (1 - Math.max(0, Math.min(1, v / maxYVal)));

        let rxPath = "";
        let rxAreaPath = "";
        if (rxSorted.length > 0) {
          const coords = rxSorted.map(p => `${getX(p.time)},${getY(p.val)}`);
          rxPath = "M " + coords.join(" L ");
          const firstX = getX(rxSorted[0].time);
          const lastX = getX(rxSorted[rxSorted.length - 1].time);
          const baseIdxY = padT + chartH;
          rxAreaPath = `M ${firstX},${baseIdxY} L ` + coords.join(" L ") + ` L ${lastX},${baseIdxY} Z`;
        }

        let txPath = "";
        let txAreaPath = "";
        if (txSorted.length > 0) {
          const coords = txSorted.map(p => `${getX(p.time)},${getY(p.val)}`);
          txPath = "M " + coords.join(" L ");
          const firstX = getX(txSorted[0].time);
          const lastX = getX(txSorted[txSorted.length - 1].time);
          const baseIdxY = padT + chartH;
          txAreaPath = `M ${firstX},${baseIdxY} L ` + coords.join(" L ") + ` L ${lastX},${baseIdxY} Z`;
        }
        ...
        const rxName = this._stripDevicePrefix(rxFriendly, deviceName, this._config?.device) || "RX";
        const txName = this._stripDevicePrefix(txFriendly, deviceName, this._config?.device) || "TX";

        host.innerHTML = `
          <svg viewBox="0 0 ${Width} ${Height}" width="100%" height="100%" style="display:block; overflow:visible; user-select:none;">
            <defs>
            ...
            <!-- Y Grid Lines -->
            ${yGridLines.map(line => `
              <line x1="${padL}" y1="${line.y}" x2="${Width - padR}" y2="${line.y}" stroke="color-mix(in srgb, var(--divider-color, rgba(255,255,255,0.08)) 30%, transparent)" stroke-dasharray="4 4"></line>
              <text x="${padL - 10}" y="${line.y + 4}" text-anchor="end" fill="var(--secondary-text-color)" style="font-size:10px; font-family:var(--primary-font-family, sans-serif);">
            ...
            <!-- X Grid/Ticks -->
            ${xTicks.map(tick => `
              <line x1="${tick.x}" y1="${padT}" x2="${tick.x}" y2="${Height - padB}" stroke="color-mix(in srgb, var(--divider-color, rgba(255,255,255,0.04)) 15%, transparent)" stroke-dasharray="2 2"></line>
              <text x="${tick.x}" y="${Height - 15}" text-anchor="middle" fill="var(--secondary-text-color)" style="font-size:10px; font-family:var(--primary-font-family, sans-serif);">
                ${tick.timeStr}
              </text>
            `).join("")}

            <!-- Y Axis Unit Title -->
            <text x="${12}" y="${padT - 10}" fill="var(--secondary-text-color)" style="font-size:10px; font-family:var(--primary-font-family, sans-serif); font-weight:600; opacity:0.85;">
              ${scaleLabel}
            </text>

            <!-- RX Path & Area -->
            ${rxAreaPath ? `<path d="${rxAreaPath}" fill="url(#rxGrad)"></path>` : ""}
            ${rxPath ? `<path d="${rxPath}" fill="none" stroke="#3b82f6" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"></path>` : ""}

            <!-- TX Path & Area -->
            ${txAreaPath ? `<path d="${txAreaPath}" fill="url(#txGrad)"></path>` : ""}
            ${txPath ? `<path d="${txPath}" fill="none" stroke="#eab308" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"></path>` : ""}

            <!-- Legends -->
            <g transform="translate(${padL}, ${padT - 15})">
              <!-- RX -->
              <circle cx="5" cy="-3" r="4" fill="#3b82f6"></circle>
              <text x="15" y="1" fill="var(--primary-text-color)" style="font-size:11px; font-family:var(--primary-font-family, sans-serif);">${rxName}</text>
              
              <!-- TX -->
              <circle cx="150" cy="-3" r="4" fill="#eab308"></circle>
              <text x="160" y="1" fill="var(--primary-text-color)" style="font-size:11px; font-family:var(--primary-font-family, sans-serif);">${txName}</text>
            </g>

            <!-- Interactive Cursor & Tooltip -->
            <line id="graph-cursor" x1="0" y1="${padT}" x2="0" y2="${Height - padB}" stroke="color-mix(in srgb, var(--primary-text-color) 30%, transparent)" stroke-width="1.5" style="display:none; pointer-events:none;"></line>
            <circle id="graph-rx-dot" r="4.5" fill="#3b82f6" stroke="white" stroke-width="1.5" style="display:none; pointer-events:none;"></circle>
            <circle id="graph-tx-dot" r="4.5" fill="#eab308" stroke="white" stroke-width="1.5" style="display:none; pointer-events:none;"></circle>

            <!-- Hitbox -->
            <rect class="chart-hitbox" x="${padL}" y="${padT}" width="${chartW}" height="${chartH}" fill="rgba(0,0,0,0)" style="cursor:crosshair; pointer-events:all;"></rect>
          </svg>
        `;

        const hitbox = host.querySelector(".chart-hitbox");
        const cursor = host.querySelector("#graph-cursor");
        const rxDot = host.querySelector("#graph-rx-dot");
        const txDot = host.querySelector("#graph-tx-dot");
        const tooltip = root.querySelector(".ssm-graph-tooltip");

        const findClosest = (points, t) => {
          if (!points || points.length === 0) return null;
          let closest = points[0];
          let minDist = Math.abs(points[0].time - t);
        ...
        if (hitbox && cursor && rxDot && txDot && tooltip) {
          const onMove = (ev) => {
            const rect = hitbox.getBoundingClientRect();
            const clientX = ev.clientX - rect.left;
            const svgX = padL + (clientX / rect.width) * chartW;
            const time = minTime + ((svgX - padL) / chartW) * timeRange;

            const rxClosest = findClosest(rxSorted, time);
            const txClosest = findClosest(txSorted, time);

            if (rxClosest || txClosest) {
              const activeTime = rxClosest ? rxClosest.time : txClosest.time;
              const xPos = getX(activeTime);

              cursor.setAttribute("x1", xPos);
              cursor.setAttribute("x2", xPos);
              cursor.style.display = "block";

              let tooltipHTML = `<div style="font-weight:700; margin-bottom:4px; border-bottom:1px solid color-mix(in srgb, var(--divider-color, rgba(255,255,255,0.1)) 40%, transparent); padding-bottom:2px; font-size:10px;">${new Date(activeTime).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit', second:'2-digit'})}</div>`;

              if (rxClosest) {
                const yPos = getY(rxClosest.val);
                rxDot.setAttribute("cx", xPos);
                rxDot.setAttribute("cy", yPos);
                rxDot.style.display = "block";
                tooltipHTML += `<div style="display:flex; align-items:center; gap:6px;"><span style="display:inline-block; width:8px; height:8px; border-radius:50%; background:#3b82f6;"></span><b>RX:</b> ${formatValue(rxClosest.val)} ${scaleLabel}</div>`;
              } else {
                rxDot.style.display = "none";
              }

              if (txClosest) {
                const yPos = getY(txClosest.val);
                txDot.setAttribute("cx", xPos);
                txDot.setAttribute("cy", yPos);
                txDot.style.display = "block";
                tooltipHTML += `<div style="display:flex; align-items:center; gap:6px;"><span style="display:inline-block; width:8px; height:8px; border-radius:50%; background:#eab308;"></span><b>TX:</b> ${formatValue(txClosest.val)} ${scaleLabel}</div>`;
              } else {
                txDot.style.display = "none";
              }

              tooltip.innerHTML = tooltipHTML;
              tooltip.style.display = "block";

              // Position tooltip relative to the modal
              const modalRect = root.querySelector(".ssm-modal").getBoundingClientRect();
              const leftPos = ev.clientX - modalRect.left + 15;
              const topPos = ev.clientY - modalRect.top - 40;
    ...
    const selScale = root.querySelector('[data-graph-scale="1"]');
    if (selScale) {
      selScale.value = this._selectedGraphScale || "auto";
      selScale.addEventListener("change", (ev) => {
        this._selectedGraphScale = ev.currentTarget.value;
        renderGraph(false); // redraw from cache
      });
    }
  ...
  _openDialog(entity_id) {
    ...
    // PoE Control Entities lookup (parallel entities for this switch port)
    const poeEntityId = `switch.${baseObj}_poe`;
    const poePriorityEntityId = `select.${baseObj}_poe_priority`;
    const poeState = this._hass?.states?.[poeEntityId];
    const poePriorityState = this._hass?.states?.[poePriorityEntityId];

    let poeControlsHtml = "";
    if (poeState) {
      const poeIsOn = (poeState.state || "").toLowerCase() === "on";
      const poeLabel = poeIsOn ? "PoE: Auto/On" : "PoE: Disabled/Off";
      const btnLabel = poeIsOn ? "Turn off" : "Turn on";
      poeControlsHtml += `
        <div class="poe-control-row">
          <b>PoE Power:</b>
          <span class="poe-status-label">${poeLabel}</span>
          <button class="btn small accent" data-poe-toggle="${poeEntityId}">${btnLabel}</button>
        </div>
      `;
    }
    if (poePriorityState) {
      const currentOption = poePriorityState.state || "Low";
      const optionsHtml = ["Critical", "High", "Low"].map(opt => {
        const sel = opt.toLowerCase() === currentOption.toLowerCase() ? " selected" : "";
      ...
      poeControlsHtml += `
        <div class="poe-control-row">
          <b>PoE Priority:</b>
          <select class="poe-priority-select" data-poe-priority="${poePriorityEntityId}">
            ${optionsHtml}
          </select>
        </div>
      `;
    ...
    this._modalStyle.textContent = `
      ...
      .ssm-modal-title {
        ...
        font-size: 20px;
        margin-bottom: 12px;
        background: linear-gradient(135deg, var(--primary-text-color) 0%, color-mix(in srgb, var(--primary-text-color) 60%, transparent) 100%);
      ...
      .ssm-modal-body div {
        ...
        font-size: 14px;
        line-height: 1.5;
        border-bottom: 1px solid color-mix(in srgb, var(--divider-color, rgba(255, 255, 255, 0.05)) 20%, transparent);
        padding-bottom: 6px;
      }
      .ssm-modal-body div:last-child {
        border-bottom: none;
      }
      .btn {
        font: inherit;
      ...
      .btn.wide {
        flex: 1;
        text-align: center;
        background: linear-gradient(135deg, var(--accent-color, #38bdf8) 0%, var(--primary-color, #0284c7) 100%);
        border: none;
        color: #ffffff;
        font-weight: 600;
      }
    ...
    this._modalEl.querySelector(".ssm-modal")?.addEventListener("click", (e) => {
      e.stopPropagation();
    });
    ...
    // Toggle PoE Power handler
    const poeToggleBtn = this._modalEl.querySelector("[data-poe-toggle]");
    if (poeToggleBtn) {
      poeToggleBtn.addEventListener("click", (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        const poeEid = ev.currentTarget.getAttribute("data-poe-toggle");
        const poeSt = this._hass?.states?.[poeEid];
        if (poeSt) {
          const isOn = (poeSt.state || "").toLowerCase() === "on";
          this._hass.callService("switch", isOn ? "turn_off" : "turn_on", { entity_id: poeEid });
        }
      });
    }

    // PoE Priority select handler
    const poePrioritySel = this._modalEl.querySelector("[data-poe-priority]");
    if (poePrioritySel) {
      poePrioritySel.addEventListener("change", (ev) => {
        const poePriEid = ev.currentTarget.getAttribute("data-poe-priority");
  ...
  _calibStorageKey() {
    ...
    // v3 key: scope to a stable per-card identifier to avoid collisions between multiple cards/switches.
    // IMPORTANT: Do NOT include title (changing title should not break persistence).
    const id = this._inferDeviceStorageId() || "unknown";
    const bg = String(this._config?.background_image || "");
    return `ssm_calib_v3:${id}:${bg}`;
  }

  _calibStorageKeyLegacyV2() {
  ...
  _inferDeviceStorageId() {
    ...
    // Anchor entity (full entity id without domain) is a stable and specific identifier.
    const ae = cfg.anchor_entity ? String(cfg.anchor_entity) : "";
  ...
  _loadCalibMapFromStorage() {
    try {
      ...
      if (!raw) {
        const keyV2 = this._calibStorageKeyLegacyV2();
        const rawV2 = localStorage.getItem(keyV2);
        if (rawV2) {
          try {
            const objV2 = JSON.parse(rawV2);
            const mapV2 = (objV2 && typeof objV2 === "object" && objV2.map && typeof objV2.map === "object") ? objV2.map
                        : (objV2 && typeof objV2 === "object") ? objV2
                        : null;

            if (mapV2 && typeof mapV2 === "object") {
              const ports = Array.isArray(this._ports) ? this._ports : [];
              const portNames = new Set(ports.map(p => String(p?.name || "").trim().toLowerCase()).filter(Boolean));
              const keys = Object.keys(mapV2).map(k => String(k || "").trim().toLowerCase()).filter(Boolean);

              // If most keys match current port names, treat it as a valid legacy layout and migrate.
              const match = keys.reduce((acc, k) => acc + (portNames.has(k) ? 1 : 0), 0);
              const ratio = keys.length ? (match / keys.length) : 0;

              if (ratio >= 0.6) {
                raw = rawV2;
                try { localStorage.setItem(keyV3, rawV2); } catch (e) {}
              }
            }
          } catch (e) {
            // ignore legacy parse/migration failures
          }
        }
      }
      ...
      if (obj && typeof obj === "object" && obj.map && typeof obj.map === "object") {
        ...
        const vb = (obj.viewbox && typeof obj.viewbox === "object") ? obj.viewbox : null;
...
_isCalibrationEnabled() {
  ...
  _setupCalibrationUI() {
    ...
    if (!this._calibMap) {
      ...
      if (stored && stored.map) {
        ...
        if (!this._calibUplinkBox && stored.uplink_box && typeof stored.uplink_box === "object") {
          const b = stored.uplink_box;
          this._calibUplinkBox = { x: Number(b.x), y: Number(b.y), w: Number(b.w), h: Number(b.h) };
        }
        if (!this._calibPortsBox && stored.ports_box && typeof stored.ports_box === "object") {
          const b = stored.ports_box;
          this._calibPortsBox = { x: Number(b.x), y: Number(b.y), w: Number(b.w), h: Number(b.h) };
        }
      } else {
    ...
    const setPortXY = (entityId, x, y) => {
      ...
      if (r) {
        ...
        if (Number.isFinite(dx) && Number.isFinite(dy) && (dx !== 0 || dy !== 0)) {
          ...
          // Move link/PoE LEDs (absolute SVG coords inside the <g>)
          if (g) {
            g.querySelectorAll(".link-led, .poe-led").forEach(led => {
              led.setAttribute("cx", String(Number(led.getAttribute("cx") || "0") + dx));
              led.setAttribute("cy", String(Number(led.getAttribute("cy") || "0") + dy));
            });
          }
        }
      }
    ...
    const applyPortsBoxLayout = () => {
      ...
      // IMPORTANT: when the Layout Editor "Ports box" tool is active, the user is explicitly
      // requesting the card to (re)pack ports inside the box. In that case we MUST ignore
      // any existing saved positions; otherwise changing Order (Odd/Even vs Numeric) will
      // appear to "do nothing" until a port is moved.
      ...
      if (_posRaw && typeof _posRaw === "object") {
        ...
        if (_hasMainPos && !this._calibPortsBoxMode) return;
      }
...
const vbObj = (this._ssmLastViewBox && this._ssmLastViewBox.w > 0 && this._ssmLastViewBox.h > 0)
  ? { w: this._ssmLastViewBox.w, h: this._ssmLastViewBox.h }
  : null;

// Update in-memory config so the preview immediately reflects the saved layout.
// Then emit a Lovelace editor event so the HA editor persists it.
try {
  const newCfg = { ...(this._config || {}), port_positions: safeMap, viewbox: vbObj || undefined };
  this._config = newCfg;
...
// Also broadcast on window — the card editor listens for this there.
try {
  window.dispatchEvent(new CustomEvent("ssm-port-positions-saved", {
    detail: {
      device: this._config?.device || "",
      background_image: this._config?.background_image || "",
      port_positions: safeMap,
      viewbox: vbObj,
    },
  }));
} catch (e) {}

// Keep a local dispatch as well for any legacy listeners.
try {
...
// Mark clean + show Saved
this._calibDirty = false;
try { setMsg("Saved"); } catch (e) {}
...
// Begin group drag for current selection
  ...
  async _render() {
    ...
    if (prefix) {
      const diagPrefix = prefix.replace(/_[a-z0-9]+\d+(_\d+)?$/i, "");
      const infoEid = Object.keys(this._hass.states).find(k => 
        k.startsWith("sensor.") && 
        (k.includes(diagPrefix) || k.includes(prefix)) && 
        (k.endsWith("_device_info") || k.endsWith("_device_information"))
      );
      if (infoEid && this._hass.states[infoEid]?.state === "unavailable") {
        isOffline = true;
      }
    }
    if (!isOffline && phys.length > 0 && phys.every(([, st]) => st.state === "unavailable")) {
      isOffline = true;
    }
    ...
    const offlineBanner = isOffline ? `
      <div class="offline-banner">
        ...
        <span>Switch Connection Lost (Offline)</span>
      </div>
    ` : "";
    ...
    const style = `
      ...
      ha-card {
        display: block;
        background: color-mix(in srgb, var(--ha-card-background, var(--card-background-color, #1e1e24)) 75%, transparent);
      ...
      .head {
        ...
        font-weight: 600;
        padding: 16px 20px;
        border-bottom: 1px solid color-mix(in srgb, var(--divider-color, rgba(255, 255, 255, 0.08)) 25%, transparent);
        color: var(--primary-text-color);
        background: linear-gradient(180deg, color-mix(in srgb, var(--primary-text-color) 2%, transparent) 0%, transparent 100%);
      }
      ...
      .port:hover {
        ...
        border-color: color-mix(in srgb, var(--primary-text-color) 15%, transparent);
...
.panel-wrap { line-height: 0; }
.panel-wrap svg { display: block; }

/* Background image: preserve aspect ratio (no stretching) and avoid "mystical" padding by
   making the wrapper match the panel's viewBox aspect ratio via --ssm-ar (set inline). */
.panel-wrap.bg {
  padding: 0;
  width: 100%;
  height: auto;
  aspect-ratio: var(--ssm-ar);
...
.calib-msg{ margin-left:8px; }
      ...
      @keyframes pulse-border {
        0%, 100% { border-color: rgba(239, 68, 68, 0.3); box-shadow: 0 4px 12px rgba(239, 68, 68, 0.1); }
        50% { border-color: rgba(239, 68, 68, 0.6); box-shadow: 0 4px 16px rgba(239, 68, 68, 0.2); }
      }
    `;
    ...
    if (diag && !phys.length && !virt.length && !this._config.ports) {
      ...
      // Re-attach modal/style if open (if not already attached to shadowRoot)
      if (this._modalStyle && !this._modalStyle.parentNode) this.shadowRoot.appendChild(this._modalStyle);
      if (this._modalEl && !this._modalEl.parentNode) this.shadowRoot.appendChild(this._modalEl);
    ...
    const panelView = () => {
      ...
      const isUplinkPort = (name, id) => {
        return separateUplinks && uplinkSet.size && _ssmIsUplinkPort(uplinkSet, name, id, this);
      };

      // Exclude hidden ports
      const visiblePorts = panelPorts.filter(([id, st]) => {
        const a = st?.attributes || {};
        const name = String(a.Name || id.split(".")[1] || "");
        return !_ssmIsHiddenPort(this._config, name, id);
      });

      const uplinks = visiblePorts.filter(([id, st]) => {
        const a = st?.attributes || {};
        const name = String(a.Name || id.split(".")[1] || "");
        return isUplinkPort(name, id);
      });

      const mainPorts = visiblePorts.filter(([id, st]) => {
        const a = st?.attributes || {};
        const name = String(a.Name || id.split(".")[1] || "");
        return !isUplinkPort(name, id);
      });
      ...
      const P = Number(this._config.port_size || 18);
      const Gh = Number(this._config.horizontal_port_gap);
      const Gv = Number(this._config.vertical_port_gap);
      const perRowCfg = Math.max(1, this._config.ports_per_row || 24);

      // Use actual physical/main port count (excluding separated uplinks) for main grid sizing
      const portCount = Math.max(1, mainPorts.length || 1);
      ...
      if (W <= 0) {
        ...
        storedVB = (() => {
          try {
            ...
            return (s && s.viewbox && Number.isFinite(Number(s.viewbox.w)) && Number.isFinite(Number(s.viewbox.h)))
              ? { w: Number(s.viewbox.w), h: Number(s.viewbox.h) }
              : null;
          } catch (e) {
            return null;
          }
      ...
      // Panel height: background image aspect ratio takes priority when known.
      // storedVB is a fallback for when the image hasn't loaded yet (first render).
      // Without a background image, storedVB preserves calibrated viewbox height.
      ...
      if (useBg) {
        ...
        if (Number.isFinite(asp) && asp > 0 && Number.isFinite(W) && W > 0) {
          // Aspect ratio known: use it, ignore stale storedVB height
          H = Math.max(140, Math.round(W / asp));
        } else if (storedVB && Number.isFinite(Number(storedVB.h)) && Number(storedVB.h) > 0) {
          // Aspect ratio not yet loaded: use stored value until image loads
          H = Number(storedVB.h);
        }
        // else: fall through to content-driven H above
      } else if (storedVB && Number.isFinite(Number(storedVB.h)) && Number(storedVB.h) > 0) {
        H = Number(storedVB.h);
      }
      ...
      if (isDynH) {
        GhStep = (perRowUsed > 1) ? (usableW - perRowUsed * Pstep) / (perRowUsed - 1) : 4;
        if (GhStep > Pstep * 2) GhStep = Pstep * 2;
        if (GhStep < 2) GhStep = 2;
      }
      if (isDynV) {
        GvStep = (rows > 1) ? (usableH - rows * Pstep) / (rows - 1) : 4;
        if (GvStep > Pstep * 2) GvStep = Pstep * 2;
        if (GvStep < 2) GvStep = 2;
      }
      ...
      const autoPackedMap = new Map();
      mainPorts.sort((a, b) => _ssmNaturalPortCompare(a[1]?.attributes?.Name || a[0].split(".")[1] || "", b[1]?.attributes?.Name || b[0].split(".")[1] || ""));
      mainPorts.forEach(([id, st], idx) => {
        const c = idx % perRowUsed;
        const r = Math.floor(idx / perRowUsed);
        const x = startX + c * (Pstep + GhStep);
        const y = topPad + r * (Pstep + GvStep);
        autoPackedMap.set(id, { x, y });
      });
      ...
      if (uplinks.length) {
        let ub = normBox(this._calibUplinkBox || (this._config.uplink_box && typeof this._config.uplink_box === "object" ? this._config.uplink_box : null));
        if (!ub) {
          ub = { x: W - sidePad - 120, y: 18, w: 120, h: 90 };
        }
        const n = uplinks.length;
        const spanX = Math.max(0, ub.w - P);
        const spanY = Math.max(0, ub.h - P);
        const ratio = (spanY > 0) ? (spanX / spanY) : (spanX || 1);

        const cols = Math.max(1, Math.round(Math.sqrt(n * ratio)));
        const rowsA = Math.max(1, Math.ceil(n / cols));
        ...
        if (isDynH) {
          GhStepA = (cols > 1) ? (ub.w - cols * P) / (cols - 1) : 4;
          if (GhStepA > P * 2) GhStepA = P * 2;
          if (GhStepA < 2) GhStepA = 2;
        }
        if (isDynV) {
          GvStepA = (rowsA > 1) ? (ub.h - rowsA * P) / (rowsA - 1) : 4;
          if (GvStepA > P * 2) GvStepA = P * 2;
          if (GvStepA < 2) GvStepA = 2;
        }

        const gridW = cols * P + Math.max(0, cols - 1) * GhStepA;
        const gridH = rowsA * P + Math.max(0, rowsA - 1) * GvStepA;

        const startXA = Number(ub.x) + Math.max(0, (ub.w - gridW) / 2);
        const startYA = Number(ub.y) + Math.max(0, (ub.h - gridH) / 2);

        uplinks.sort((a, b) => _ssmNaturalPortCompare(a[1]?.attributes?.Name || a[0].split(".")[1] || "", b[1]?.attributes?.Name || b[0].split(".")[1] || ""));
        uplinks.forEach(([id, st], idx) => {
          const c = idx % cols;
          const r = Math.floor(idx / cols);
          const x = startXA + c * (P + GhStepA);
          const y = startYA + r * (P + GvStepA);
          autoPackedMap.set(id, { x, y });
        });
      }
      ...
      if (this._isCalibrationEnabled() && this._calibAutoGridEnabled) {
        ...
        if (metaChanged) {
          try {
            ...
            const ordered = visiblePorts.map(([id, st], i) => {
              const a = st?.attributes || {};
              const name = String(a.Name || id.split(".")[1] || "");
              return { id, st, name, i };
            ...
            const uplinks = (separateUplinks && uplinkSet.size) ? ordered.filter(p => _ssmIsUplinkPort(uplinkSet, p.name, p.id, this)) : [];
            const mainPorts = (separateUplinks && uplinks.length) ? ordered.filter(p => !_ssmIsUplinkPort(uplinkSet, p.name, p.id, this)) : ordered;

            const nextMap = {};

            let pb = normBox(this._calibPortsBox);
            if (!pb) {
              pb = { x: sidePad, y: 18, w: usableW, h: usableH };
            }

            const portsOrder = String(this._calibPortsOrder || "numeric");
            const applySnap = (val) => {
              const snap = Number(this._calibSnap) || 0;
              return snap > 0 ? Math.round(val / snap) * snap : val;
            };
            ...
            if (portsOrder === "odd_even") {
              ...
              const rowGroupSize = hasEvenRow ? 2 : 1;
              const groups = Math.max(1, Math.ceil(maxLen / cols));
              const rowsA = groups * rowGroupSize;

              let GhStepA = Number(this._config.horizontal_port_gap);
              let GvStepA = Number(this._config.vertical_port_gap);
              const isDynH = !(GhStepA > 0);
              const isDynV = !(GvStepA > 0);

              if (isDynH) {
                GhStepA = (cols > 1) ? (pb.w - cols * P) / (cols - 1) : 4;
                if (GhStepA > P * 2) GhStepA = P * 2;
                if (GhStepA < 2) GhStepA = 2;
              }
              if (isDynV) {
                GvStepA = (rowsA > 1) ? (pb.h - rowsA * P) / (rowsA - 1) : 4;
                if (GvStepA > P * 2) GvStepA = P * 2;
                if (GvStepA < 2) GvStepA = 2;
              }
              ...
              const startXA = Number(pb.x) + Math.max(0, (pb.w - gridW) / 2);
              const startYA = Number(pb.y) + Math.max(0, (pb.h - gridH) / 2);
              ...
              const place = (arr, rowOffset) => {
                arr.forEach((p, i) => {
                  ...
                  const x = startXA + c * (P + GhStepA);
                  const y = startYA + r * (P + GvStepA);
                  const roundedX = applySnap(x);
                  const roundedY = applySnap(y);
                  nextMap[String(p.name)] = { x: roundedX, y: roundedY };
                  nextMap[String(p.id.split(".")[1] || "")] = { x: roundedX, y: roundedY };
                });
              };
              ...
              mainPorts.sort((a, b) => _ssmNaturalPortCompare(a.name, b.name));

              const n = mainPorts.length;
              const cap = Math.max(1, parseInt(this._config?.ports_per_row, 10) || 24);
            ...
            // Separated Uplinks Box layout re-packer
            let ub = normBox(this._calibUplinkBox);
            if (uplinks.length) {
              if (!ub) {
                ub = { x: W - sidePad - 120, y: 18, w: 120, h: 90 };
              }
      ...
      const storedMap2 = _stored ? _stored.map : null;
      const portPosRaw = (this._isCalibrationEnabled() && this._calibMap && typeof this._calibMap === "object")
        ? this._calibMap
        : ((storedMap2 && typeof storedMap2 === "object")
            ? storedMap2
            : ((this._config.port_positions && typeof this._config.port_positions === "object")
                ? this._config.port_positions
                : null));
      const portPos = portPosRaw
        ? new Map(Object.entries(portPosRaw).map(([k, v]) => {
      ...
      if (this._config.show_labels && _labelPosMode === "split") {
        let _minY = Infinity;
        let _maxY = -Infinity;
        for (let _i = 0; _i < visiblePorts.length; _i++) {
          const [_id, _st] = visiblePorts[_i];
          ...
          if (portPos) {
            ...
            if (_ov && typeof _ov === "object") {
              ...
              if (Number.isFinite(_ox)) { _x = _ox; _hasPos = true; }
              if (Number.isFinite(_oy)) { _y = _oy; _hasPos = true; }
            }
          }
          if (!_hasPos) {
            const _pt = autoPackedMap.get(_id);
            if (_pt) {
              _x = _pt.x;
              _y = _pt.y;
            } else {
              const _idx = _i % perRowUsed, _row = Math.floor(_i / perRowUsed);
              _x = startX + _idx * (Pstep + GhStep);
              _y = topPad + _row * (Pstep + GvStep);
            }
          }
      ...
      const rects = visiblePorts.map(([id, st], i) => {
        const a = st.attributes || {};
        const name = String(a.Name || id.split(".")[1] || "");
        ...
        if (portPos) {
          ...
          if (ov && typeof ov === "object") {
            ...
            if (Number.isFinite(ox)) { x = ox; hasPos = true; }
            if (Number.isFinite(oy)) { y = oy; hasPos = true; }
          }
        }
        if (!hasPos) {
          const pt = autoPackedMap.get(id);
          if (pt) {
            x = pt.x;
            y = pt.y;
          } else {
            const idx = i % perRowUsed, row = Math.floor(i / perRowUsed);
            x = startX + idx * (Pstep + GhStep);
            y = topPad + row * (Pstep + GvStep);
          }
        }
        ...
        const hasPoe = (a && Number(a["PoE Power (W)"]) > 0);
        const operState = String(a.Oper || "").toLowerCase();
        const hasLink = (operState === "up" || operState === "1");
        
        let ledColor = "transparent";
        let ledGlow = "";
        
        if (hasLink) {
          const speed = String(a.Speed || "").toLowerCase();
          if (speed.includes("gbps") || speed.includes("g")) {
            ledColor = "#22c55e"; // bright emerald green
            ledGlow = "filter:drop-shadow(0 0 2px #22c55e);";
          } else if (speed.includes("mbps") || speed.includes("m")) {
            ledColor = "#f59e0b"; // amber orange
            ledGlow = "filter:drop-shadow(0 0 2px #f59e0b);";
          } else {
            ledColor = "#22c55e"; // default active green
            ledGlow = "filter:drop-shadow(0 0 2px #22c55e);";
          }
        } else if (operState === "down" || operState === "2") {
          ledColor = "#ef4444"; // red for admin down/disconnected
          ledGlow = "opacity:0.35;";
        } else {
          ledColor = "#4b5563"; // offline charcoal
          ledGlow = "opacity:0.25;";
        }

        const ledSize = Math.max(3, Math.round(Ps * 0.18));
        const r_led = ledSize / 2;
        const offset = r_led + 1.2;
        const linkLed = `
          <circle class="link-led" cx="${x + Ps - offset}" cy="${y + offset}" r="${r_led}"
            fill="${ledColor}" stroke="rgba(0,0,0,0.8)" stroke-width="0.8" style="${ledGlow} pointer-events:none"/>
        `;
        const poeLed = hasPoe ? `
          <circle class="poe-led" cx="${x + offset}" cy="${y + offset}" r="${r_led}"
            fill="#3b82f6" stroke="rgba(0,0,0,0.8)" stroke-width="0.8" style="filter:drop-shadow(0 0 2px #3b82f6); pointer-events:none"/>
        ` : "";
      ...
      // Remember the final panel viewBox so the Save button can persist it.
      // This keeps autoscale stable across HA Edit mode vs normal mode.
      this._ssmLastViewBox = { w: W, h: H };
      ...
      // Ensure background wrapper uses the same aspect ratio as the panel viewBox (prevents letterboxing gaps).
      if (useBg) {
        _wrapStyles.push(`--ssm-ar:${W} / ${H}`);
      }
        ...
        ${this._isCalibrationEnabled() ? `
          <div class="calib-tools">
            ...
            <div class="calib-actions">
              ...
              <button class="iconbtn ${this._calibPortsBoxMode ? "active" : ""}" id="ssm-calib-portsbox" type="button" title="Ports"><ha-icon icon="mdi:ethernet"></ha-icon></button>
              ${this._config.show_uplinks_separately ? `<button class="iconbtn ${this._calibUplinkBoxMode ? "active" : ""}" id="ssm-calib-uplinkbox" type="button" title="Uplinks"><ha-icon icon="mdi:arrow-up-bold"></ha-icon></button>` : ``}
    ...
    this._cardEl.querySelectorAll(".btn[data-entity]").forEach(btn => {
    ...
    this._cardEl.querySelectorAll("[data-alias-entity]").forEach(el => {
    ...
    this._cardEl.querySelectorAll(".port-svg[data-entity]").forEach(g => {
      g.addEventListener("pointerdown", (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        const id = g.getAttribute("data-entity");
